...
/// Find all approved proposals marked ready for agenda (ready_for_agenda="true").
/// Returns proposals sorted by submitted_date DESC.
pub fn find_ready_for_agenda(
    conn: &Connection,
    tor_id: i64,
...
/// Find all proposals that have been scheduled into a specific agenda point.
/// Returns proposals linked via spawns_agenda_point relation.
pub fn get_queue_for_agenda_point(
    conn: &Connection,
    agenda_point_id: i64,
) -> Result<Vec<ProposalListItem>, AppError> {
    let mut stmt = conn.prepare(
        "SELECT e.id, \
                ...
                r_spawn.source_id AS related_suggestion_id \
         FROM entities e \
         JOIN relations spawns_ap ON e.id = spawns_ap.source_id \
         JOIN entities spawns_ap_rt ON spawns_ap.relation_type_id = spawns_ap_rt.id \
            AND spawns_ap_rt.name = 'spawns_agenda_point' \
         LEFT JOIN entity_properties p_title \
             ON e.id = p_title.entity_id AND p_title.key = 'title' \
         LEFT JOIN entity_properties p_date \
             ON e.id = p_date.entity_id AND p_date.key = 'submitted_date' \

...
fn http_get(url: &str) -> String {
    let output = Command::new("curl")
        .args(&["-s", "-L", "-c", "/tmp/cookies.txt", "-b", "/tmp/cookies.txt", url])
        .output()
...
fn http_post(url: &str, form_data: &[(&str, &str)]) -> String {
    ...
    let output = Command::new("curl")
        .args(&["-s", "-L", "-c", "/tmp/cookies.txt", "-b", "/tmp/cookies.txt"])
        .arg("-d")
        .arg(&form_str)
        .arg(url)
        .output()
...
fn test_can_view_outlook_calendar() {
    ...
    let login_html = http_get("http://localhost:8080/login");
    let csrf_token = extract_csrf_token(&login_html)
        .expect("Failed to extract CSRF token");
    ...
    let _login_response = http_post("http://localhost:8080/login", &login_data);
    println!("[TEST] ✓ Logged in");
    
    // Step 2: Access the outlook calendar endpoint
    println!("[TEST] Step 2: Fetching outlook calendar...");
    let html = http_get("http://localhost:8080/tor/outlook");
...
fn test_can_create_and_confirm_projected_meeting() {
    ...
    println!("[TEST] Step 1: Getting login page...");
    let login_html = http_get("http://localhost:8080/login");
    let csrf_token = extract_csrf_token(&login_html)
        .expect("Failed to extract CSRF token from login page");
    println!("[TEST] ✓ Got CSRF token");
    
    // Step 2: Login
    println!("[TEST] Step 2: Logging in as admin...");
    let login_data = vec![
        ("username", "admin"),
        ("password", "admin123"),
        ("csrf_token", &csrf_token),
    ...
    // Step 3: Get ToR list to find a ToR
    println!("[TEST] Step 3: Getting ToR list...");
    let tor_list = http_get("http://localhost:8080/tor");
    assert!(
    ...
    // Step 4: Get ToR detail page to extract CSRF for meeting form
    println!("[TEST] Step 4: Getting ToR detail page...");
    let tor_url = format!("http://localhost:8080/tor/{}", tor_id);
    let tor_detail = http_get(&tor_url);
    ...
    // Step 5: Create a projected meeting
    println!("[TEST] Step 5: Creating projected meeting...");
    ...
    let confirm_response = http_post(&meeting_url, &meeting_data);
    ...
    // Step 6: Navigate to outlook calendar
    println!("[TEST] Step 6: Navigating to outlook calendar...");
    let calendar = http_get("http://localhost:8080/tor/outlook");
    ...
    // Step 7: Verify projected meeting has correct CSS class
    println!("[TEST] Step 7: Verifying projected meeting styling...");
    assert!(
    ...
    // Step 8: Verify confirm button is present
    println!("[TEST] Step 8: Verifying confirm button...");
    assert!(
...
fn test_projected_vs_confirmed_styling() {
    ...
    // Step 1: Fetch outlook calendar
    println!("[TEST] Step 1: Fetching outlook calendar...");
    let calendar = http_get("http://localhost:8080/tor/outlook");
    
    // Step 2: Verify both projected and confirmed CSS classes exist as options
    println!("[TEST] Step 2: Checking CSS class definitions...");
    ...
    // Step 3: Verify the calendar HTML structure is correct
    println!("[TEST] Step 3: Verifying calendar HTML structure...");
    assert!(
    ...
    // Step 4: Verify JavaScript is loaded
    println!("[TEST] Step 4: Verifying JavaScript is loaded...");
    assert!(
...
fn test_calendar_event_data_structure() {
    ...
    println!("[TEST] Step 1: Logging in...");
    let login_html = http_get("http://localhost:8080/login");
    let csrf_token = extract_csrf_token(&login_html).expect("Failed to extract CSRF");
    let login_data = vec![
        ("username", "admin"),
        ("password", "admin123"),
        ("csrf_token", &csrf_token),
